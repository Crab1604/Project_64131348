...
using Project_64131348.Controllers;
using Project_64131348.Models;


namespace Project_64131348.Controllers
{
    public class NhanViens_64131348Controller : Base_64131348Controller
    {
        ...
        // GET: NhanViens
        [HasCredentia(IDQuyen = "QUANLYNHANVIEN")]
        ...
        public ActionResult Search(string maNV = "", string tenNV = "", string gioiTinh = "", string diaChi = "", string email = "", string sdt = "", string idNhom = "")
        {
            ...
            var nhanViens = db.NhanViens.SqlQuery("KhachHang_TimKiem'" + maNV + "','" + tenNV + "','" + gioiTinh + "','" + diaChi + "','" + email + "','" + sdt + "','" + idNhom + "'");
        ...
        [ValidateAntiForgeryToken]
        [HasCredentia(IDQuyen = "QUANLYNHANVIEN")]
        public ActionResult Edit([Bind(Include = "maNV,tenNV,gioiTinh,ngaySinh,diaChi,email,sdt,hinhAnh")] NhanVien nhanVien, HttpPostedFileBase Avatar)
        {
            if (Avatar != null && Avatar.ContentLength > 0)
            {
                try
                {
                    ...
                    var path = Path.Combine(Server.MapPath("~/Images/"), postedFileName);
                    Avatar.SaveAs(path);
                    nhanVien.hinhAnh = postedFileName; // Lưu tên file vào thuộc tính hinhAnh
                }
                catch (Exception ex)
                {
                    ModelState.AddModelError("", "Có lỗi khi tải lên ảnh: " + ex.Message);
                }
            }
